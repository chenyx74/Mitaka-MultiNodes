...
then
	...
	fn_log "openstack user create cinder --password cinder"
...
then
	...
	openstack endpoint create --publicurl http://${NAMEHOST}:8776/v2/%\(tenant_id\)s --internalurl http://${NAMEHOST}:8776/v2/%\(tenant_id\)s --adminurl http://${NAMEHOST}:8776/v2/%\(tenant_id\)s --region RegionOne volume  
	openstack endpoint create --publicurl http://${NAMEHOST}:8776/v2/%\(tenant_id\)s --internalurl http://${NAMEHOST}:8776/v2/%\(tenant_id\)s --adminurl http://${NAMEHOST}:8776/v2/%\(tenant_id\)s --region RegionOne volumev2
...
openstack-config --set /etc/cinder/cinder.conf  database connection  mysql://cinder:CINDER_DBPASS@${NAMEHOST}/cinder 
...
openstack-config --del /etc/cinder/cinder.conf  DEFAULT lock_path
openstack-config --del /etc/cinder/cinder.conf  DEFAULT iscsi_helper
...
echo -e "\033[32m ###         install cinder complete successfull      #### \033[0m"
echo -e "\033[32m ######################################################### \033[0m"
