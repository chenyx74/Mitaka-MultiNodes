...
#This scripts enhanced by shan jin xiao at 2015/11/11(single day)!
...
yum clean all && yum install openstack-selinux -y && yum install mariadb mariadb-server MySQL-python -y
fn_log "yum clean all && yum install openstack-selinux mariadb mariadb-server MySQL-python -y"
...
then 
  ...
  echo "there is no openstack user in rabbit!"
	fn_install_rabbit
fi
}
...
then
	...
	echo  "mairadb had installation successful."
else
	fn_install_mariadb
fi
