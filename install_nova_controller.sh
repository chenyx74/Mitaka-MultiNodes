...
mysql -uroot -proot -e "CREATE DATABASE nova;" &&  mysql -uroot -proot -e "GRANT ALL PRIVILEGES ON nova.* TO 'nova'@'localhost' IDENTIFIED BY 'NOVA_DBPASS';" && mysql -uroot -proot -e "GRANT ALL PRIVILEGES ON nova.* TO 'nova'@'%' IDENTIFIED BY 'NOVA_DBPASS';" 
...
read -p "please choose your controler node management NIC number[default first NIC]:" NIC_NUM
if [ -z ${NIC_NUM} ]
then
...
openstack-config --set  /etc/nova/nova.conf database connection  mysql://nova:NOVA_DBPASS@${NAMEHOST}/nova 
...
openstack-config --set  /etc/nova/nova.conf DEFAULT vncserver_listen  ${NIC_IP} 
openstack-config --set  /etc/nova/nova.conf DEFAULT vncserver_proxyclient_address  ${NIC_IP} 
openstack-config --set  /etc/nova/nova.conf DEFAULT verbose  True 
openstack-config --set  /etc/nova/nova.conf DEFAULT debug  True
openstack-config --set  /etc/nova/nova.conf glance host  ${NAMEHOST} 
...
#unset http_proxy https_proxy ftp_proxy no_proxy 
read -p "please choose your nova-compute vnc listen NIC number[default same as management NIC]:" NIC_NUM
if [ -z ${NIC_NUM} ]
then
	echo "use default the first NIC as your nova management IP"
	NIC_NUM=${NIC_NUM} 
...
openstack-config --set  /etc/nova/nova.conf DEFAULT novncproxy_base_url  http://${NIC_IP}:6080/vnc_auto.html
